...
int main()
{
    string grid = "08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08\
                   ...
                   20 73 35 29 78 31 90 01 74 31 49 71 48 86 81 16 23 57 05 54\
                   01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48";
    vector<vector<int> > grid_processed;
    vector<int> row_processed;
    int start = 0;
    for (unsigned int i = 0; i < grid.size(); i++) {
        if(grid[i] == ' ') {
            string word = grid.substr(start, 2);
            if (word[0] != ' ' && word[1] != ' ') {
                row_processed.push_back(stoi(grid.substr(start, 2)));
            }
            if (row_processed.size() == 20) {
                grid_processed.push_back(row_processed);
                row_processed.clear();
            }
            start = i + 1;
        }
